...
struct APIGrowth {
    ...
    func runDailyReview(_ body: [String: Any]) async throws -> GrowthReport {
        try await client.post("/api/v2/growth/daily-review", body: AnyEncodable(body),
            mock: { MockGrowth.dailyReview })
    }

    func listReports(limit: Int = 20) async throws -> [GrowthReport] {
        try await client.get("/api/v2/growth/reports?limit=\(limit)", mock: MockGrowth.reports)
    }

    func listCandidates(limit: Int = 20) async throws -> [StrategyCandidate] {
        try await client.get("/api/v2/growth/candidates?limit=\(limit)", mock: MockGrowth.candidates)
    }
    ...
    func confirmCandidate(_ id: String) async throws -> StrategyCandidate {
        try await client.post("/api/v2/growth/candidates/\(id)/confirm", body: AnyEncodable([String: String]()),
            mock: { MockGrowth.confirmed(id: id) })
    }
}

enum MockGrowth {
    static var dailyReview: GrowthReport {
        GrowthReport(
            id: UUID().uuidString, reportType: "daily_review", status: "completed",
            summary: AnyCodable([
                "report_date": "2026-06-05", "total_strategies_reviewed": 8, "candidates_found": 3,
                "top_performer": "RSI Mean Reversion", "top_performer_pnl": 1245.80,
                "avg_win_rate": 0.62, "avg_sharpe_ratio": 1.35,
                "recommendations": [
    ...
    static func reports() -> [GrowthReport] {
        [
            dailyReview,
            GrowthReport(
                id: UUID().uuidString, reportType: "daily_review", status: "completed",
                summary: AnyCodable([
                    "report_date": "2026-06-04", "total_strategies_reviewed": 7, "candidates_found": 1,
                    "top_performer": "MACD Crossover", "avg_win_rate": 0.58,
                ] as [String: Any]),
                createdAt: "2026-06-04T06:00:00Z"
            ),
            GrowthReport(
                id: UUID().uuidString, reportType: "daily_review", status: "failed",
                summary: nil, createdAt: "2026-06-03T06:00:00Z"
            ),
        ]
    }

    static func candidates() -> [StrategyCandidate] {
        [
            StrategyCandidate(
                id: UUID().uuidString, status: "pending_review", confidence: 0.82,
                createdAt: "2026-06-05T06:15:00Z",
                dsl: AnyCodable(["name": "Volume Breakout v2", "symbol": "SOL/USDT",
                    "source_type": "ai_generated", "backtest_win_rate": 0.68, "backtest_sharpe": 1.72,
                    "reasoning": "Strong volume profile breakout pattern on 4H timeframe"] as [String: Any])
            ),
            StrategyCandidate(
                id: UUID().uuidString, status: "confirmed", confidence: 0.91,
                createdAt: "2026-06-04T14:30:00Z",
                dsl: AnyCodable(["name": "Funding Rate Arb", "symbol": "BTC/USDT",
                    "source_type": "manual", "backtest_win_rate": 0.82, "backtest_sharpe": 2.10,
                    "reasoning": "Exploits funding rate divergence"] as [String: Any])
            ),
            StrategyCandidate(
                id: UUID().uuidString, status: "rejected", confidence: 0.35,
                createdAt: "2026-06-03T20:00:00Z",
                dsl: AnyCodable(["name": "Momentum Scalper", "symbol": "ETH/USDT",
                    "source_type": "ai_generated", "backtest_win_rate": 0.51, "backtest_sharpe": 0.45,
                    "reasoning": "Rejected — poor risk-adjusted returns"] as [String: Any])
            ),
        ]
    }

    static func confirmed(id: String) -> StrategyCandidate {
        StrategyCandidate(
            id: id, status: "confirmed", confidence: 0.82,
            createdAt: "2026-06-05T06:15:00Z",
            dsl: AnyCodable(["name": "Volume Breakout v2", "symbol": "SOL/USDT",
                "reasoning": "Confirmed for dryrun deployment"] as [String: Any])
        )
    }
}
...
extension APIGrowth {
    func getShapFeatures() async throws -> ShapFeaturesResponse {
        try await client.get("/api/growth/shap-features", mock: MockGrowth.shapFeatures)
    }

    func getSignalValidity() async throws -> SignalValidityResponse {
        try await client.get("/api/growth/signal-validity", mock: MockGrowth.signalValidity)
    }
}

extension MockGrowth {
    static func shapFeatures() -> ShapFeaturesResponse {
    ...
    static func signalValidity() -> SignalValidityResponse {
