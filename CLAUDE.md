...
- **`Views/BacktestAndDryrun/BacktestLabView`** — Backtest & simulation page as three-column linked-flow layout (2026-07-01 refactor, supersedes the old 9-section narrative). Left Run Rail (240pt, history + compare checkboxes + New Run), center phase-driven column (Backtest/Dryrun tab switch + inline `ConfigPanel` + `StatusSummaryBlock` / `EquityCurveBlock` / `TradeListBlock` / `CompareBlock`), right Context Rail (280pt, always-visible `StrategyMetaPanel` / `RiskWarningsPanel` / `PromotionGatePanel`). Driven by `BacktestLabViewModel` with `Phase` state machine (idle/configuring/running/completed/failed) + `activeTab: RunTab` + `submittedConfig: RunConfig?`. Center result blocks render only when `phase == .completed`; CompareBlock only when ≥2 runs checked. Real backend data only — no client-side PRNG equity curve; honest empty states when backend returns none. Mock mode (decided globally in `AlphaLoopApp`, not per-ViewModel) shows `MOCK` badge. Block views live under `Center/`, `LeftRail/`, `RightRail/`; pure helpers under `Shared/` (`RunFailureClustering.swift`, `RiskWarningRules.swift`, `SectionCard.swift`). PromotionGate CTA jumps to `.liveReadiness` route; this page never starts live trading directly. Backend persistence: `backtest_handler._handle_success` writes `result["equity_curve"]` + `result["trades"]`; `FreqtradeBacktestRunner._parse_result` normalizes freqtrade trade keys to `TradeRow` schema and derives the equity curve.
